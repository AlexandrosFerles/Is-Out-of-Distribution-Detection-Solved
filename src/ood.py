...
def _gen_odin_inference(model, loaders, device, ind_dataset, val_dataset, ood_dataset, exclude_class=None):
    ...
    model.eval()
    val_ind_loader, test_ind_loader, val_ood_loader, test_ood_loader = loaders
    _score_classification_accuracy(model, test_ind_loader, device, dataset=ind_dataset, genOdin=True)
...
if __name__ == '__main__':
    ...
    loaders = get_ood_loaders(batch_size=args.batch_size, ind_dataset=args.in_distribution_dataset, val_ood_dataset=args.val_dataset, test_ood_dataset=args.out_distribution_dataset, exclude_class=args.exclude_class)
