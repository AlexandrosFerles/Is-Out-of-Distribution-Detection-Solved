...
def train(dv):
    ...
    strides = [1, 1, 2, 2]
    model = WideResNet(d=40, k=2, n_classes=10, input_features=3, output_features=16, strides=strides)
    ...
    epochs = 200
    scheduler = MultiStepLR(optimizer, milestones=[int(0.3*epochs), int(0.5*epochs), int(0.8*epochs)], gamma=0.2)
    ...
    for epoch in tqdm(range(epochs)):
        ...
        if epoch_accuracy > best_test_acc:
            ...
            torch.save(model.state_dict(), f'/raid/ferles/checkpoints/wide/wide_resnet_epoch_{epoch}_acc_{best_test_acc}.pth')
